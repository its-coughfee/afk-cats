// AFK-cats — everything the Worker does, with no Cloudflare-specific imports.
//
// The sign page's HTML arrives as a parameter rather than an import, so this
// file can be loaded by Node's test runner as ordinary JavaScript. `src/index.js`
// is the thin wrapper that imports the page and hands it over.
//
// Three jobs:
...
export async function handleRequest(request, env, signHtml) {
  ...
  if (request.method === 'GET' && url.pathname.startsWith('/sign/')) {
    return handleSignPage(url.pathname.slice('/sign/'.length), env, signHtml);
  }
...
async function handleSignPage(signId, env, signHtml) {
  if (!isSignId(signId)) return new Response('Not found', { status: 404 });
  const record = await getSign(env, signId);
  if (!record) return new Response('Not found', { status: 404 });

  const page = signHtml.replace(
    '</head>',
...
export function newSignId() {
  const bytes = crypto.getRandomValues(new Uint8Array(16));
  return [...bytes].map((b) => b.toString(16).padStart(2, '0')).join('');
}

export function isSignId(value) {
...
export function wallClockToMs(tz, year, month, day, h, m) {
...
export function nextOccurrence(tz, h, m, nowMs) {
  ...
  // Tomorrow is today's date plus one, not the clock plus 24 hours: on the night
  // the clocks go forward, 24 hours from now is already past midnight of the day
  // after, which would set the sign a day late.
  const next = new Date(Date.UTC(today.year, today.month - 1, today.day + 1));
  return wallClockToMs(tz, next.getUTCFullYear(), next.getUTCMonth() + 1, next.getUTCDate(), h, m);
}

// "3:00 PM BST". The clock is always formatted for a US English reader; the zone
// name is looked up separately because en-US only knows letter abbreviations for
// American zones and falls back to "GMT+1" everywhere else.
export function formatInZone(ms, tz) {
  const clock = new Intl.DateTimeFormat('en-US', {
    timeZone: tz,
    hour: 'numeric',
    minute: '2-digit',
  }).format(new Date(ms));
  return `${clock} ${zoneName(ms, tz)}`;
}

// The first locale that gives letters rather than a GMT± offset wins. Some zones
// have no abbreviation in any of them, and there the offset form is the answer.
